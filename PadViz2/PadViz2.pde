// PadViz2 — Paddle stroke visualiser (Euler-angle approach)
// Processing 4.x
...
void keyPressed() {
    ...
    if (key == '0')  { testMode = 0; surface.setTitle("PadViz2"); return; }
    if (key == '1')  { testMode = 1; surface.setTitle("PadViz2 — TEST: roll only"); return; }
    if (key == '2')  { testMode = 2; surface.setTitle("PadViz2 — TEST: pitch only"); return; }
    if (key == '3')  { testMode = 3; surface.setTitle("PadViz2 — TEST: yaw only"); return; }
