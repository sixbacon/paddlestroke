...
### PadDis (RX — CYD, COM14 as of 13 Jul 2026 — port moves, check `arduino-cli board list`)

```bash
arduino-cli compile firmware/production/PadDis/
arduino-cli upload -p COM14 firmware/production/PadDis/
...
- **Zero-feather CPM fix (20 Jul 2026, spec §16.12)** — pitch-fed `CadenceACF` (new `CadenceACF.h/.cpp`, ported from `visualisation/stroke_acf.py`) BUILT into PadLog v8.10 / PadDis v8.14, both compile clean; **not yet flashed or bench-verified**. Payload's reserved pad byte becomes `cpm_source` (0=peak detector white, 1=ACF zero-feather fallback yellow `0x07FF`, 2=arbiter-suppressed `?? cpm`); payload size unchanged at 80 B. Offline acceptance test (`visualisation/stroke_zero_feather_regression.py`) PASSES both cases: 16 Jul zero-feather segment reports source=1 at 32.7 CPM (spec table: 32.8) for 100% of settled samples; 11 Jul padbad case (via the CSV's own recorded `cpm` vs a fresh pitch-ACF estimate, matching `stroke_acf.py`'s already-validated methodology) flags 100% disagreement, matching spec §16.9. Arbiter threshold fixed at the tested 30% (not the plan text's "~25%" approximation). **Still needed:** on-hardware 20-test sim suite additions, bench test, field test — all require physical hardware.
...
| PadDis | `firmware/production/PadDis/` | CYD ESP32-2432S028 | COM14 | `esp32:esp32:esp32` |
| BoatLog | `firmware/production/BoatLog/` | LOLIN32 Lite | COM3 | `esp32:esp32:lolin32-lite` |

**Version scheme:** `<phase>.<iteration>` — PadLog **v8.9**, PadDis **v8.13**, BoatLog **v1.2**. Versions can diverge when only one sketch changes; Phase 10 and the GRV release required coordinated bumps per spec §15.6 (payload struct changes).
