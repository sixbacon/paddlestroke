...
- **GRV data collection (14 Jul 2026, spec §16.11)** — coordinated release PadLog v8.9 / BoatLog v1.2 / PadDis v8.12: BNO085 Game Rotation Vector (mag-free quaternion) at 100 Hz on both TX units, appended to both payloads (paddle 64→80 B, boat 74→90 B) and both CSVs (`grv_qw..qz` / `boat_grv_qw..qz`, appended at end). Data collection only — feeds the future Phase 9 fused-vs-GRV relative-yaw decision, since GRV is not reconstructible offline. Compiled clean; bench verification pending (spec §16.11 checklist).
...
**Version scheme:** `<phase>.<iteration>` — PadLog **v8.9**, PadDis **v8.12**, BoatLog **v1.2**. Versions can diverge when only one sketch changes; Phase 10 and the GRV release required coordinated bumps per spec §15.6 (payload struct changes).
