// PadViz3 — Paddle stroke visualiser (quaternion approach)
// Processing 4.x
...
FrameData applyTestMode(FrameData src) {
    ...
    // Recompute quaternion from the masked Euler angles so Model3D uses correct quat
    fd.recomputeQuat();
    return fd;
}

// ── HUD overlay (drawn on main P2D canvas) ────────────────────────────────────
...
void keyPressed() {
    ...
    if (key == '0')  { testMode = 0; surface.setTitle("PadViz3"); return; }
    if (key == '1')  { testMode = 1; surface.setTitle("PadViz3 — TEST: roll only"); return; }
    if (key == '2')  { testMode = 2; surface.setTitle("PadViz3 — TEST: pitch only"); return; }
    if (key == '3')  { testMode = 3; surface.setTitle("PadViz3 — TEST: yaw only"); return; }
