...
# ---------- cycle segmentation ----------
lp = fft_band(roll, FS, 0.2, 2.0)
peaks = find_extrema(lp, int(1.0 * FS), 30.0)
troughs = find_extrema(-lp, int(1.0 * FS), 30.0)
print(f'right1: {len(peaks)} roll peaks, {len(troughs)} troughs, '
      f'{ (t[-1]-t[0])/60:.1f} min')
...
fig.suptitle('right1 segment - stroke-phase ensembles (peak-anchored = side A, '
...
for col, anchors in enumerate((peaks, troughs)):
    ...
    axq.plot(phx, md, 'k')
    axq.set_ylabel('roll (deg)' if col == 0 else '')
    axq.set_title(f'{"side A (roll peak)" if col==0 else "side B (roll trough)"} '
                  f'n={len(ens_roll)}')
...
axes[-1, 0].set_xlabel('stroke phase (0 = roll peak)')
axes[-1, 1].set_xlabel('stroke phase (0 = roll trough)')
plt.tight_layout()
out = os.path.join(HERE, 'catch_ensemble.png')
...
ax2[2].set_xlabel('s')
out2 = os.path.join(HERE, 'catch_strip.png')
...
ax3[0].plot(phx, np.median(e_rl, 0), 'k'); ax3[0].set_ylabel('roll deg')
...
ax3[2].set_xlabel('stroke phase (0 = roll peak)')
plt.tight_layout()
out3 = os.path.join(HERE, 'tip_height.png')
