...
## 15.5 Standalone clear-classification key (`q`, v0.27, 12 Aug 2026)

The only way to undo classification used to be the wizard's Step-4 reset
(`r/R`, §14). But the wizard auto-completes after Step 5 and then reopens
**read-only** (F1/`w` show a summary, never re-enter the steps), so once a
session was set up there was no live way to correct a bad classification short
of restarting the sketch and reloading, or hand-editing the sidecar's
`classification_sections` array.

`q` (also `Q`) fixes that. It is a global key — active in any slice — that
clears **all** classification sections, rewrites the sidecar
(`saveSidecarQuiet()`), and rebuilds the exclusion/visibility index so any
frames previously excluded by a `d/D` section become navigable again. **Roll
calibration is untouched** (only `sidecar.classification` is emptied).

Because a full wipe is destructive and the key is always live, it uses a
**two-press confirm**: the first `q` arms and flashes
`PRESS q AGAIN TO CLEAR N CLASSIFICATION SECTION(S)`; a second `q` within
`CLEAR_CLASS_CONFIRM_MS` (3 s) commits and flashes
`CLASSIFICATION CLEARED (N sections removed)`. Pressing `q` with nothing
classified flashes `NO CLASSIFICATION TO CLEAR`. A new **CLASSIFICATION**
group in the Commands menu carries a `q` row ("Clear ALL classification (press
twice)") — the menu synthesises the same keystroke, so a menu click arms then
commits on a second click, exactly like the keyboard path.

Implementation: `clearClassificationRequested()` + `clearClassArmedMs` /
`CLEAR_CLASS_CONFIRM_MS` in `PadViz8.pde`; menu row in `Menu.pde`. BUILT +
compiles clean; the two-press flow and the resulting graph/navigation refresh
are **runtime behaviour still to be confirmed on screen** (per the
compile-vs-runtime caveat — `--build` does not exercise key dispatch).
