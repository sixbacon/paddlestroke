...
## 15.6 Track view respects classification (v0.29, 12 Aug 2026)
...
bounding box until the actual paddling area was a tiny knot. Classification
already excludes the drive home from the graph and all navigation (§14); the
Track view now honours it too.

Classification is defined in **paddle** frames, while the track is boat lat/lon,
so the filter bridges the two via `SyncMap` (paddle→boat, `boatIdxFor`).
`TrackPanel.rescan()` builds, over the boat frames, `boatExcl[]` / `boatKeep[]`
by walking every paddle frame: a boat frame is treated as excluded only if an
**excluded** paddle frame maps to it **and no non-excluded paddle frame also
does** (so a boundary boat fix straddling the edge of an excluded range is
kept). Each valid boat fix landing in an excluded boat frame is dropped; boat
fixes with no paddle mapping (e.g. a boat-only lead-in) are kept, since the
classification says nothing about them.

Crucially, the **bounds are recomputed from the kept fixes** (not
`BoatSource.latLonRange()` over the whole file), so the zoom fits the valid
track. The live-position dot and the green start / red end markers therefore
also reference the valid track. The filter engages only when
`classify.hasExclusions()` and paddle+sync are present; otherwise the view is
exactly as before ("every valid boat fix"). The footer shows
`N fixes (excluded ranges hidden)` when filtering is active, and the empty case
reads "No GPS fixes left after classification (all excluded?)".

Cache: `rescan()` is memoised on the boat frame count, so changing
classification alone wouldn't re-filter — `rebuildClassificationIndex()` now
calls `trackView.invalidate()` so the scan is redone whenever the sections
change (wizard edit, `q` clear, session load).

Implementation: `TrackPanel.rescan(BoatSource, SyncMap)` + `filteredByClass`
flag in `TrackPanel.pde`; `trackView.invalidate()` from
`rebuildClassificationIndex()` in `PadViz8.pde`. BUILT + compiles clean;
on-screen confirmation pending (needs a session whose sidecar carries an
excluded range — compile-vs-runtime caveat).
