...
//
// Three mutually-exclusive full-window views share the sketch: the main 3D
// visualisation, the side-profile blade window (SideProfilePanel, key x) and
// the GPS track window (TrackPanel, key t, v0.26). This bar draws a clickable
// tab for each, so they can be switched with the mouse as well as the keyboard
// — the active view's tab is highlighted. Clicks route through the same
// showView() helper the x/t keys and Commands menu use, so keyboard and mouse
...
class Tabs {
    ...
    // 2 = track (GPS over an OSM backdrop, v0.26).
    final String[] labels = { "3D VIEW", "SIDE PROFILE", "TRACK" };
    ...
    int activeTab() { return sideActive() ? 1 : trackActive() ? 2 : 0; }
    ...
    boolean mousePressed(int mx, int my) {
        ...
        for (int i = 0; i < labels.length; i++) {
            ...
            if (mx >= tx && mx <= tx + TAB_W) {
                showView(i);   // 0 = 3D scene, 1 = side profile, 2 = track
                return true;
            }
        }
        return false;
