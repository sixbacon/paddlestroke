...
// When a boat frame is available, the kayak is oriented by its full quaternion.
// When no boat frame is available, the kayak is oriented by paddle avgYaw only.
...
class Model3D {
    ...
    void draw(FrameData fd, BoatFrameData bfd,
        ...
        // Kayak — use boat quaternion if available, else avgYaw only
        canvas.pushMatrix();
        if (bfd != null) {
            float[] qk = { bfd.kayakQw, bfd.kayakQx, bfd.kayakQy, bfd.kayakQz };
            applyQuat(canvas, qk);
        } else {
        ...
        // Paddle
        canvas.pushMatrix();
        if (posMode) canvas.translate(posX * S, 0, 0);
    ...
    private void setCamera(float avgYaw, BoatFrameData bfd) {
        float headingDeg = (bfd != null) ? bfd.kayakYaw : avgYaw;
        if (deckCam) {
            float a    = radians(headingDeg);
            float dist = 3.25f * S;
            float dz   = -0.4f * S;
    ...
    private void applyQuat(PGraphics g, float[] q) {
        ...
        float m00 = 1 - 2*(y*y + z*z);  float m01 =     2*(x*y - w*z);  float m02 =     2*(x*z + w*y);
        float m10 =     2*(x*y + w*z);  float m11 = 1 - 2*(x*x + z*z);  float m12 =     2*(y*z - w*x);
        float m20 =     2*(x*z - w*y);  float m21 =     2*(y*z + w*x);  float m22 = 1 - 2*(x*x + y*y);
