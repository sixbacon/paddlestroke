...
class Sidecar {
    ...
    // Classification sections (PadViz7 v0.16, spec §14) — which stretches of
    // this recording are good right/left/zero-feather paddling, or excluded.
    // Persisted as the "classification_sections" JSON array. Independent of
    // the rest-window calibration above.
    ArrayList<ClassificationSection> classification = new ArrayList<ClassificationSection>();
    ...
    void save(String path) {
        ...
        for (ClassificationSection cs : classification) {
            JSONObject o = new JSONObject();
            o.setInt("start_frame", cs.startFrame);
            o.setInt("end_frame",   cs.endFrame);
            o.setString("type",     classTypeName(cs.type));
            cls.append(o);
        }
        root.setJSONArray("classification_sections", cls);
    ...
    boolean loadFromFile(String path) {
        try {
            ...
            if (root.hasKey("classification_sections")) {
                JSONArray cls = root.getJSONArray("classification_sections");
                for (int i = 0; i < cls.size(); i++) {
                    JSONObject o = cls.getJSONObject(i);
                    int t = classTypeFromName(o.getString("type"));
                    if (t < 0) continue;
                    classification.add(new ClassificationSection(
                        o.getInt("start_frame"), o.getInt("end_frame"), t));
                }
            }
